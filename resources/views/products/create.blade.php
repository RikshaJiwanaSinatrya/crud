...
<div class="max-w-2xl mx-auto fade-in">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-extrabold text-surface-900 tracking-tight">Tambah Produk Baru</h1>
        <p class="text-surface-500 text-sm mt-1">Isi informasi produk di bawah ini.</p>
    </div>
        ...
        <form action="{{ route('products.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <textarea id="description"
                          ...
                          class="w-full px-4 py-3 bg-surface-50 border border-surface-200 rounded-xl text-surface-800 text-sm placeholder:text-surface-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-colors resize-none">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1.5 font-medium">{{ $message }}</p>
                @enderror
            </div>
